...
课程格式会生成真正的二进制 UDP 数据，而不是 `name=value` 文本。界面预览使用大写、空格分隔的十六进制摘要，例如：

```text
HEX: 12 34 0A FF
```

SQLite 不逐包保存载荷，而是在一次发送完成、被用户停止或失败时写入一条任务汇总，包含配置数量、实际总数、频率、开始/结束时间、目标 IP、目标端口、状态和错误信息。

## 课程字段约束

- `isSelected` 和 `isKey` 必须明确写成 `true` 或 `false`；
- `isKey=true` 的标识字段必须提供 `data`，并使用该固定值；
- `isKey=false` 的非标识字段根据 `minimum`、`maximum` 和 `precision` 随机生成，即使 XML 中存在 `data` 也不把它当固定值；
- `isSelected=false` 只表示“不关注”，不会把字段从实际报文位布局中删掉；
- `DEC/INT/UINT/OCT/HEX/STRING` 的长度必须是 8 的倍数，`FLAG` 长度必须为 8，`FLT/DBL` 长度必须为 64，`IP` 长度必须为 32；
- 本轮暂不把最终“14 种类型名单”作为验收重点，现有兼容类型继续保留。
